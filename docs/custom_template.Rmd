...
*Relative Importance Ranking (N = 38)*
...
rank_data |>
  ...
  annotate("segment", 
           x = 8,
           xend = 7.7, 
           y = deframe(rank_data[rank_data$dim1 =="First destination data" & rank_data$`Institution Name` == params$college,'ranking']), 
           yend = 5, 
           colour = "black")+
...
all_list$multi$Q9 |>
  ...
  serviceTab(q = "Q9", 
             ...
             subtitle = "For write-in responses, see Appendix",
             offer = "Conferences")
```
...
sengage_type_data |>
  ...
  tableViz(var = 'engage', subtitle = "School Averages", college = params$college)
```
...
### Professional Staff Appt w/ Students
...
info_no_data |>  
  ggplot(aes(reorder(`Institution Name`,amount),amount, fill = dim2))+
  
  geom_hline(yintercept = c(0,200,400,600,800), colour= "grey")+
...
info_no_data |>
  ...
  tableViz(var = 'amount', college = params$college)
```
...
`r glue::glue("{write_in_con[!is.na(write_in_con)]}")`

#### Undergraduate Services
`r glue::glue("{write_in_userve[!is.na(write_in_userve)]}", .sep = "\n")`


#### Undergraduate Programs
`r glue::glue("{write_in_uprogram[!is.na(write_in_uprogram)]}", .sep = "\n")`


#### Graduate Services
`r glue::glue("{write_in_gserve[!is.na(write_in_gserve)]}", .sep = "\n")`
