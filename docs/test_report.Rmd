...
all_list$multi$Q10 |>
  ...
  dplyr::mutate(
    ...
    freq = format(round(freq,2), nsmall = 2), .keep = "unused"
  ) |>
  dplyr::arrange(-n)|>
...
appt_student_data |>
  ...
  ggplot2::ggplot(ggplot2::aes(Year,amount))+
...
appt_student_data |>
  group_by(`Institution Name`) |>
  summarise(Appt = sum(Appt)) |>
  
  tableViz(var = 'Appt', subtitle = "All Years")
```
...
total_budget |>
  ...
  scale_fill_manual(values=c("#6AD4BF","#F7BF66"),
                    breaks=c("Total_NOP","Total_OP"),
                    labels=c("Non-Operating", "Operating"))+
